        ...
        .then(res => {
          let playlists = res.data.data.collects.map(item => {
            return {
              id: item.listId,
              cover:
                item.collectLogo.replace('http://', 'https://') + '@!c-185-185',
              coverBig:
                item.collectLogo.replace('http://', 'https://') + '@!c-185-185',
              coverSmall:
                item.collectLogo.replace('http://', 'https://') + '@!c-185-185',
        ...
        .then(res => {
          let albumList = res.data.data.albums.map(item => {
            return {
              id: item.albumId,
              cover:
                item.albumLogo.replace('http://', 'https://') +
                '@1e_1c_0i_1o_100Q_250w_250h',
              coverBig:
                item.albumLogo.replace('http://', 'https://') +
                '@1e_1c_0i_1o_100Q_400w_400h',
              coverSmall:
                item.albumLogo.replace('http://', 'https://') +
                '@1e_1c_0i_1o_100Q_150w_150h',
              name: item.albumName,
        ...
        let songList = ab.songs.map(item => {
          ...
          return {
            ...
            album: {
              ...
              id: ab.albumId,
              cover:
                ab.albumLogo.replace('http://', 'https://') +
                '@1e_1c_0i_1o_100Q_250w_250h',
              coverBig:
                ab.albumLogo.replace('http://', 'https://') +
                '@1e_1c_0i_1o_100Q_400w_400h',
    ...
    newRequest('mtop.alimusic.music.list.collectservice.getcollectdetail', {
      ...
      .then(res => {
        ...
        let songList = pl.songs.map(item => {
          ...
          return {
            ...
            album: {
              ...
              cover:
                item.albumLogo.replace('http://', 'https://') +
                '@0e_0c_0i_1o_100Q_250w',
              coverBig:
                item.albumLogo.replace('http://', 'https://') +
                '@0e_0c_0i_1o_100Q_400w',
              coverSmall:
                item.albumLogo.replace('http://', 'https://') +
                '@0e_0c_0i_1o_100Q_150w',
              name: item.albumName
            }
          };
        });
        let obj = {
          ...
          id: pl.listId,
          cover:
            pl.collectLogo.replace('http://', 'https://') +
            '@0e_0c_0i_1o_100Q_250w',
          coverBig:
            pl.collectLogo.replace('http://', 'https://') +
            '@0e_0c_0i_1o_100Q_400w',
        ...
        const songList = songs.map(item => {
          ...
          return {
            album: {
              ...
              coverSmall:
                item.albumLogo.replace('http', 'https') + '@!c-185-185'
            },
            artists: [
              {
                name: item.artistName,
